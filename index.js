...
client.on("ready", () => {
  ...
  client.user.setActivity(`#- Alx S`, { type: 'WATCHING' })
});
...
const staffManagerRole = "1163720375901884417"
const discorsLeader = "1163720376803668068"
const OfficialRole = "1163720392549072946"
const discordStaff = "1163720388304453642"

const developerId = "333945284692279297"
const bankid =  "333945284692279297"
let sellermention = '1163720412560113664'
const prefix = "$"
const lineLink = "https://cdn.discordapp.com/attachments/1163720604961222739/1167700143244582922/13-7-2_1_3.png?ex=654f1480&is=653c9f80&hm=46f2b42b51dcf3f75b10f4864fca735159d40af7dcbbd6d08abdcc45330716bc&"

//////////////
client.on('messageCreate', message => {
  if (message.author.bot) return;
  if (message.channel.id == `1163741792886341642`){
...
client.on(`messageCreate`, Kimo => {
  ...
  if (command === prefix + `mtr`) {
    const rolesList = [
      { name: 'Role 1', id: '1163720411360546868' },
      { name: 'Role 2', id: '1163720410773336104' },
      { name: 'Role 3', id: '1163720409800261682' },
      { name: 'Role 4', id: '1163720408953000007' },
      { name: 'Role 5', id: '1163720407954763848' },
      { name: 'Role 6', id: '1163720407229141053' },
      { name: 'Role 7', id: '1163720406235090964' }
...
let autoline_channel = ['1163720663689859092','1163720664923000852','1163720665904459786','1163720666994974720','1163720667976450080','1163720669649965106','1163720670958600202','1163720672216883291','1163720673475178506','1163720674628616243'] // ايدي الرومات
...
message.channel.send(`<:955334:1154455565578285207> **كان معاك الاداري** <@${message.author.id}>
 <:955334:1154455565578285207> **يرجئ تقييمي هنا** <#1163720682123821146>
 <:955334:1154455565578285207> ****فضلاً وليس أمراً <a:1_rtb_A6:1149282623445798912> ****`)
...
client.on('interactionCreate', async interaction => {
    var member;
    let role = interaction.guild.roles.cache.find(role => role.id === "1163720388304453642"); // ايدي رول اللى يستلم التكت
    await interaction.deferUpdate();
    if(interaction.isButton){
    
        if(interaction.customId === `idk`){
          if(interaction.member.roles.cache.some(role => role.id === "1163720388304453642")) // حط هنا ايدي الرتبة اللى تستلم التكت
            {
        ...
        db.add(`alluser_${interaction.member.id}`, 1)
          ...
          let l1 = interaction.user;
           ...
           collector.on('collect', async i => {
            ...
            if(interaction.member.roles.cache.some(role => role.id === "1163720388304453642"))
              {
...
interaction.channel.setName("ticket-" + interaction.user.username);                interaction.channel.permissionOverwrites.edit(role,{SEND_MESSAGES: true})
          ...
          interaction.channel.permissionOverwrites.edit('1163720388304453642',{SEND_MESSAGES: false})
          interaction.channel.permissionOverwrites.edit('1163720388304453642',{VIEW_CHANNEL: true})
          }else{
            interaction.followUp({ 
                content: `ماشفتك تستخدم الزر ؟`,
                ephemeral: true 
...
client.on("messageCreate", async message => {
  if (message.content.startsWith(prefix + "ticket(+)")) {
    if (message.member.roles.cache.some(r=> r.id == 1163720359074349189)) {
      let user = message.mentions.members.first()
...
const warnWork = `1163751230640754729`
client.on("messageCreate" , message => {
  if(message.content.startsWith(prefix + "تحذير")) {
    ...
    let madri = `(=)`
    let channel = client.channels.cache.get('1163720662511271956');
    let warn2 = client.channels.cache.get('1163720661177487371');
            ...
            const memberList = await message.guild.members.fetch();
              let usersData = [];
                ...
                memberList.forEach((member) => {
                    if (member.roles.cache.has('1163720359074349189')) {
                ...
                const channel = message.guild.channels.cache.get('1163720604961222739');
    ...
    if (message.content.startsWith('$start')) {
        ...
        try {
            ...
            memberList.forEach(async (member) => {
                if (member.roles.cache.has('1163720388304453642')) {  //discord staff
                    var points = db.get(`weekuser_${member.id}`)
                    var weekwarns = db.get(`weekwarns_${member.id}`)
                    ...
                    weekmute = parseInt(weekmute) || 0;
                    messageCount++;
                    var roleToAssign = "1163720388304453642"; //discord staff
